...
export class UI {
  ...
  private trim: { x: number; y: number; width: number; height: number } | null = null;

  private fieldsets: HTMLElement[] = [];

  constructor(parameters: Parameters, pixelLogic: PixelLogic, generator: Generator) {
    ...
    this.exportUnit = document.getElementById('export-unit') as HTMLSelectElement;
    this.dpiRow = document.getElementById('dpi-row') as HTMLElement;
    this.outputPlaceholder = document.getElementById('output-placeholder') as HTMLElement;
  ...
  private toggleDpiInput() {
    this.dpiRow.style.display = this.exportUnit.value === 'cm' ? 'block' : 'none';

    // Force recalculation of maxHeight for the Export fieldset if it's expanded
    const exportFieldset = document.getElementById('fieldset-export') as HTMLElement;
